...
const ReportCard = ({ title, imageUrl, onGenerate, isLoading }: ReportData) => {
  return (
    <div
      className="w-full max-w-xs p-4 flex flex-col justify-between items-center border-2 border-gray-200 shadow-sm rounded-3xl 
    ...
    >
      ...
      <button
        onClick={onGenerate}
        disabled={isLoading}
        className="mt-6 w-full sm:w-auto px-12 py-3 
                        bg-white border border-gray-200 rounded-2xl shadow-sm 
                        text-sm font-black text-primary 
                        transition-all duration-300 
                        hover:bg-primary hover:text-white cursor-pointer 
                        active:scale-95 shrink-0 disabled:opacity-50 disabled:cursor-not-allowed"
      >
        {isLoading ? "Se generează..." : "Generare"}
      </button>
    </div>
  );
};
